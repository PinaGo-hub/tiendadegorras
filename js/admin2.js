...
const productos = [
  ...
  { id: 5, nombre: "Gorra Nike pro edición 2", precios: { "Única": 800 }, imagen: "img/card5.png" },
  { id: 6, nombre: "Gorra Adidas edición 2", precios: { "Única": 750 }, imagen: "img/card6.png" }
];
...
productos.forEach(p => {
  ...
  // Precio principal (como solo hay 1 presentación, siempre es ese)
  const preciosArray = Object.values(p.precios);
  const precioMayor = Math.max(...preciosArray);

  col.innerHTML = `
    <div class="card hoverable" style="border-radius:10px;">
      <div class="card-image">
        <img src="${p.imagen}" style="height:auto; border-radius:10px; object-fit:cover;">
      </div>
      ...
      <div class="card-action center teal darken-1" style="border-radius:10px;">
        <a href="#!" class="white-text" onclick="agregarDirecto(${p.id})">Agregar al carrito</a>
      </div>
    </div>`;
...
function agregarDirecto(id) {
  ...
  const presentaciones = Object.keys(prod.precios);
  const presentacion = presentaciones[0];
  const precioSeleccionado = prod.precios[presentacion];
  const cantidad = 1;

  const existente = carrito.find(item => item.id === prod.id && item.presentacion === presentacion);
  if (existente) {
